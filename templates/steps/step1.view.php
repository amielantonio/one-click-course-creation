    ...
    <select name="course-content" class="oc-form-control select2" id="course-content">
        ...
        <?php if(isset($courseContent)) : ?>
            <?php foreach( $courseContent as $key => $value) : ?>
                <option id="option-<?php echo $key?>" value="<?php echo $key?>" data-lessons='<?php echo json_encode($value['lessons']);?>' data-course-meta='<?php echo json_encode(($value['post_meta']));?>'><?php echo $key . " - " . $value['course_name']?></option>
            <?php endforeach;?>
        <?php endif; ?>
    </select>
</div>
